...
function BenefitMarquee({ items, scroll = true }: { items: [string, string][]; scroll?: boolean }) {
  ...
  return (
    <div style={{
      ...
      <motion.div key={isMobile ? "mobile" : "desktop"} style={{ display: "flex", width: "max-content" }} animate={reduceMotion ? {} : { x: ["0%", "-50%"] }} transition={reduceMotion ? undefined : { duration: isMobile ? 5 : 35, ease: "linear", repeat: Infinity }}>
        {pill}{pill}
      </motion.div>
    </div>
  );
